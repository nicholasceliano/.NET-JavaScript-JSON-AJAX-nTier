...
namespace Hess.Corporate.GHGPortal.Business
{
    ...
    public class StandardValidation : BusinessObjects<StandardValidation, StandardValidationRecord>
    {
        ...
        public static StandardValidation GetStdMaster(int yearNum, int monthNum, string entName = "", string facName = "", string scName = "", string astName = "", string dsourceName = "", string astType = "", string valChk = "")
        {
            Criteria _criteria = new Criteria(yearNum, monthNum, entName, facName, scName, astName, dsourceName, astType, valChk);
            return DataPortal.Fetch<StandardValidation>(_criteria);
        }

        public void Save()
        {
            foreach (StandardValidationRecord svr in this)
            {
                svr.Save();
            }
        }
        ...
        protected override void DataPortal_Fetch(object criteria)
        {
            ...
            StandardValidation stdValidation = new StandardValidation();
            ...
            using (Csla.Data.SafeDataReader dataReader = new Csla.Data.SafeDataReader(
                new DataAccess().ExecuteStoredProcedure("prcVPeGridStdMasterGrid", cr.Entity, cr.Facility, cr.SourceCategory, cr.YearNumber, cr.MonthNumber, cr.AssetName, cr.DataSource, cr.AssetType, cr.Validated)))
            {
                while (dataReader.Read())
                {
                    StandardValidationRecord record = new StandardValidationRecord(dataReader);
                    this.Add(record);
                }
            }
        }
